...
| `web/model/scope.py` | Rebuilt | Core | this change | One immutable filter value composes folders, star floors, sets, and explicit selections; large id selections remain one bound JSON argument rather than growing SQL |
| `web/model/sets.py` | Rebuilt | Organise | this change | Collections and keywords are named decision families with validated stable ids and atomic hash-membership writes; nonexistent computed-query storage was deleted rather than preserved |
...
| `web/test_core.py` | Rebuilt | Core proof | `284ba3f2`, `da1ccd7e`, `0639ab8d`, `bce1178f`, `7227545d`, this change | Opens an empty catalog and proves a real JPEG travels from fresh drive sweep through browse, full identity, and a 400 px persisted tile; import/copy/backup and set/scope refusals are included; 71 tests and 4 subtests pass on Windows, but real desktop proof is still owed |
