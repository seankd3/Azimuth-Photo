...
| Desktop UI | Designing | One fast native-feeling surface with one state and interaction vocabulary | The V2 library runs in the real native window with one store and lens registry. A disposable 47,000-photo proof rendered only 24 visible controls, jumped directly from the first page to the last while retaining 400 sparse records, moved selection and loupe with the keyboard, then released the catalog on close. Cull/Undo behavior and physical deletion of the inherited browser shell remain |
...
| `web/templates/v2.html` | Rebuilt | Desktop UI | `b153c471`, `e757a6fe`, `9ddb91a4` | The semantic desktop document names the library, native folder setup, browse controls, inspector, and loupe without inherited templates, inline behavior, or a manual page-loading control |
| `web/static/v2/index.css` | Rebuilt | Desktop UI | `b153c471`, `e757a6fe`, `9ddb91a4` | One neutral visual vocabulary keeps chrome subordinate to photographs, positions the bounded virtual cells, preserves full images in the loupe, honors reduced motion, and has no obsolete path-entry or load-more controls |
...
| `web/static/v2/store/index.js` | Rebuilt | Desktop UI | `b153c471`, `9ddb91a4` | Owns the sole mutable UI state, sparse indexed photo map, selection position, total count, and lens registry; duplicate or absent lenses fail immediately instead of silently replacing behavior |
...
| `web/static/v2/lens/library.js` | Rebuilt | Desktop UI | `b153c471`, `e757a6fe`, `9ddb91a4` | The Library lens reconciles only the visible overscan neighborhood, preserves overlapping cells, requests absent sparse ranges and near-viewport tiles, and updates accessible selection without rebuilding images |
| `web/static/v2/shell/app.js` | Rebuilt | Desktop UI | `b153c471`, `e757a6fe`, `9ddb91a4` | The shell coordinates one store and lens, direct product verbs, native folder choice, progressive refresh, sparse pages, sorting, density, race-safe details, layered Escape, grid arrows, first/last jumps, and loupe navigation; a 47,000-photo native proof loaded only the first and last pages |
