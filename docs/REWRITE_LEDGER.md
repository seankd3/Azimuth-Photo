...
| Rank, search, and AI | Designing | Decisions drive rank; enrichment improves results but never gates the product | Refine is rebuilt and proven: a round is one whole decision (this one, out of these), strength is fitted from the log, sets are drawn only from photographs whose tile exists and shown at equal area, a pair is one orientation, Undo retracts a round by saying so, and Best sorts the grid by a ranking rewritten whole off the interactive lane. Taste (the fitted direction) waits for the embedding space to become a V2 cache kind; search, captions, faces and the inherited Elo-stars projection remain |
...
| `web/model/decisions.py` | Rebuilt | Core | `284ba3f2`, `155d450c`, `8c3f24a1`, `5769f3a3` | Append-only decisions record provenance, apply one authority rule, preserve provenance when carried, expose safe partial amendment, name capture-date corrections explicitly, and say once (`PROJECTED`) which families are also browse columns with their defaults and what counts as a value |
...
| `web/rank.py` | Rebuilt | Rank, search, and AI | `2fd8ab16`, `170b9f32` | Ranking as pure functions over the decision log: `rounds` reads every round whole (and drops one a later row retracts), `strength` fits one softmax model so set size is evidence not a mode, `candidates` is the one mosaic query over any scope (least-judged anchor, nearest in rating, a pair one orientation, what is on screen avoided), `judged` is the progress, `record`/`retract` write a round and take it back by saying so; refuters hold order-independence, the prior, retraction, the ready scope and the pair rule |
