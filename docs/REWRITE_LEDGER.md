...
|---|---|---|---|
| Verification and fresh boot | Designing | One trustworthy cross-platform check; a fresh catalog opens the real desktop product | An isolated V2 local engine now opens `azimuth-v2.db`, starts the thread-confined product, serves the V2 library shell and only V2 APIs, and releases its Windows handle on prepare-quit; the packaged server still launches V1 and native-window proof remains |
...
| Desktop UI | Designing | One fast native-feeling surface with one state and interaction vocabulary | The first V2 library slice now runs against the real V2 catalog: first-folder setup, progressive scan, bounded grid, sorting, density, details, and loupe use one store and an inverted lens registry. Native folder selection, keyboard/cull behavior, virtualized whole-library proof, and physical deletion of the inherited shell remain |
...
| `web/boot.py` | Rebuilt | Verification and fresh boot | `e331be8f`, `155d450c`, `e697d24e`, `e3335a42`, this change | One `Library` owns the product verbs, including counts; `OwnedLibrary` confines interactive SQLite access to one thread, sweeps on one separate bounded connection, drains both lanes before close, and refuses work after close |
...
| `web/routes/library.py` | Rebuilt | Browse, disk, and tiles | `e697d24e`, `a3bbd3cd`, `e3335a42`, this change | Typed V2 routes serve counts, drives, refresh, bounded photo pages, embedded details, and content-keyed JPEG tiles; choosing a folder is immediate and its refresh cannot block reads |
| `web/routes/system.py` | Rebuilt | Desktop UI transport | `5c3d24eb`, this change | Local health, the desktop document, route truth, and prepare-quit are read-time answers; prepare-quit closes the owned product before acknowledging the desktop shell |
| `web/v2_app.py` | Rebuilt | Verification and fresh boot | `5c3d24eb`, this change | One lifespan constructs and starts the isolated V2 product, mounts only V2 routers and V2 static files, exposes no server documentation surface, and deterministically closes on shutdown |
| `web/templates/v2.html` | Rebuilt | Desktop UI | this change | The one semantic desktop document names the library, folder setup, browse controls, inspector, and loupe without inherited templates or inline behavior |
| `web/static/v2/index.css` | Rebuilt | Desktop UI | this change | One neutral visual vocabulary keeps chrome subordinate to photographs, uses a predictable crop grid, preserves full images in the loupe, and honors reduced motion |
| `web/static/v2/index.js` | Rebuilt | Desktop UI | this change | The composition root loads registered lenses before the shell without making the shell import a product surface |
| `web/static/v2/net/index.js` | Rebuilt | Desktop UI | this change | Owns every V2 API literal and the sole fetch boundary; failed responses become one explicit error path |
| `web/static/v2/store/index.js` | Rebuilt | Desktop UI | this change | Owns the sole mutable UI state and lens registry; duplicate or absent lenses fail immediately instead of silently replacing behavior |
| `web/static/v2/lens/library.js` | Rebuilt | Desktop UI | this change | The Library lens renders bounded photo cells, loading and empty states, and selected-photo facts with DOM nodes rather than HTML strings, then registers itself without a shell dependency |
| `web/static/v2/shell/app.js` | Rebuilt | Desktop UI | this change | The shell coordinates one store, one registered lens, typed transport, progressive drive refresh, paging, selection, sorting, density, and loupe behavior without importing a lens or V1 module |
...
| `web/test_v2_routes.py` | Rebuilt | Desktop UI transport proof | `e697d24e`, `a3bbd3cd`, `e3335a42`, this change | Refutes malformed-scope widening, index loss, cross-thread SQLite access, cancelled-close handle leaks, scan-blocked browsing, invalid tile recipes, and fake transport data; 8 tests and 6 subtests pass against a real fresh V2 catalog |
| `web/test_v2_app.py` | Rebuilt | Verification and fresh boot | `5c3d24eb`, this change | Proves the default V2 catalog/cache cannot collide with inherited state, the V2 document and module graph are served, the real route graph mounts, prepare-quit releases the catalog handle, and post-close health is unavailable |
