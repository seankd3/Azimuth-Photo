...
| Desktop UI | Designing | One fast native-feeling surface with one state and interaction vocabulary | Whole-library virtualization is proven at 47,000 photos. The native Trash lens then proved live counts, selection, Restore, exact Undo, layered Escape, and the disabled typed-count deletion gate against a disposable real-photo catalog; no destructive UI action was automated. Pick/Reject/Clear now run from the keyboard and contextbar in the native window, patch every row of an identity, and undo exactly; the grid does not yet learn of identity or metadata landing after a sweep without a reload, and physical deletion of the remaining inherited browser shell remains |
...
| `web/model/trash.py` | Rebuilt | Keep and safety | `d0dbf737`, `c78d0c32`, `8c3f24a1` | Trash is now only Restore, count, page and Empty: the status decision moved to `cull.py` (173 lines left with it). Empty preflights all registered drives, hinted and canonical copies, regular-file identity and full bytes; record copies leave last and interrupted deletion resumes from committed copy truth |
...
| `web/boot.py` | Rebuilt | Verification and fresh boot | `e331be8f`, `155d450c`, `e697d24e`, `e3335a42`, `b153c471`, `e757a6fe`, `d0dbf737`, `c78d0c32`, `8c3f24a1` | One `Library` owns browse, tile, Trash, restore, Undo and empty verbs; `OwnedLibrary` confines interactive SQLite access to one thread, sweeps on one separate bounded connection, drains both lanes before close, and refuses work after close |
...
| `web/desktop.py` | Rebuilt | Desktop packaging | `e757a6fe`, `d0dbf737`, `c78d0c32`, `8c3f24a1` | Owns platform paths, one V2 product, one native WebView2 window, direct browse and Trash verbs, the native folder chooser, and idempotent shutdown; source paths passed real-window and catalog-release proof |
...
| `web/static/v2/net/index.js` | Rebuilt | Desktop UI | `b153c471`, `e757a6fe`, `a33df983`, `8c3f24a1` | Waits for the native bridge and exposes the sole browse and Trash vocabulary; it contains no fetch, URL, route, or HTTP concept |
...
| `web/static/v2/kit/page-cache.js` | Rebuilt | Desktop UI | `9ddb91a4`, `d2a8357e` | One sparse page owner deduplicates in-flight reads, rejects stale sort generations, jumps to arbitrary offsets, turns one failed page into one visible error rather than a retry storm, and patches or removes loaded rows in place by predicate so an identity held by several rows changes on all of them |
| `web/static/v2/lens/library.js` | Rebuilt | Desktop UI | `b153c471`, `e757a6fe`, `9ddb91a4`, `a33df983`, `d2a8357e` | One lens reconciles only the visible overscan neighborhood for Library or Trash, with view-specific empty truth, the pick flag and accessible selection without image rebuilds; the inspector names the cull state and says *Reading…* until identity has arrived |
| `web/static/v2/shell/app.js` | Rebuilt | Desktop UI | `b153c471`, `e757a6fe`, `9ddb91a4`, `a33df983`, `d2a8357e` | The coordinator owns view switching, sparse pages, direct product verbs, native folder choice, sorting, density, race-safe details, layered Escape, grid keys, loupe navigation and the P/U/X cull keys, delegating the destructive conversation and the Undo toast; it offers culling only for an identified photograph and reloads when a Reject took more rows than the window holds |
| `web/static/v2/shell/trash.js` | Rebuilt | Desktop UI | `a33df983`, `d2a8357e` | One focused workflow owns Restore, preflight, the typed-count dialog and permanent outcome copy, handing Undo to the shared toast; its native proof changed 3→2→3 without moving bytes and left the destructive submit disabled |
| `web/static/v2/shell/cull.js` | Rebuilt | Desktop UI | `d2a8357e` | One small workflow turns P/U/X and the contextbar verbs into one product call, edits the loaded window in place by identity, removes a rejected cell and selects what slides into its place, and hands each change to the shared Undo toast; proven in the native window with both duplicate cells flagging, clearing, leaving (45 of 45, Trash 2) and returning on Undo |
| `web/static/v2/shell/undo.js` | Rebuilt | Desktop UI | `d2a8357e` | The one Undo toast: shows a message with the exact changes it can reverse, runs `undoCull` once and reloads; shared by Trash restore and culling instead of living inside the Trash dialog |
...
| `web/test_core.py` | Rebuilt | Core proof | `284ba3f2`, `da1ccd7e`, `0639ab8d`, `bce1178f`, `7227545d`, `e331be8f`, `84d832f2`, `155d450c`, `e3335a42`, `e757a6fe`, `d0dbf737`, `c78d0c32`, `8c3f24a1`, `d2a8357e` | Adds Trash refuters for prior-state restore, stale Undo, invalid-member atomicity, 5,000-photo batches, duplicate identities, away drives, stale copy hints, changed bytes, exact counts, record-copy-last interruption and resume; the focused V2 suite passes 99 tests on Windows |
...
| `web/test_desktop.py` | Rebuilt | Desktop packaging | `e757a6fe`, `d0dbf737`, `c78d0c32`, `8c3f24a1` | The real direct bridge crosses attach, refresh, browse, tile, Trash page/count, Undo and destructive dry-run, then closes idempotently and releases the catalog |
