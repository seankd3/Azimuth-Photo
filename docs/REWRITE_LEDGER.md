...
| Keep and safety | Designing | Trash, backup truth, recovery, and integrity checks use the same copy facts | Trash is now one reversible identity decision with exact stale-safe Undo. Empty Trash requires an unchanged typed count, every drive attached, canonical-address discovery beyond copy hints, full-byte proof, record-copy-last deletion, and resumable partial outcomes. The old 2,103-line Trash implementation is deleted; recovery, integrity, and duplicate surfaces remain inherited |
...
| `web/model/trash.py` | Rebuilt | Keep and safety | `d0dbf737`, `c78d0c32` | Reversible Trash/Restore/Undo are atomic content-identity decisions, including one 5,000-photo transaction. Empty preflights all registered drives, hinted and canonical copies, regular-file identity and full bytes; record copies leave last and interrupted deletion resumes from committed copy truth |
...
| `web/boot.py` | Rebuilt | Verification and fresh boot | `e331be8f`, `155d450c`, `e697d24e`, `e3335a42`, `b153c471`, `e757a6fe`, `d0dbf737`, `c78d0c32` | One `Library` owns browse, tile, Trash, restore, Undo and empty verbs; `OwnedLibrary` confines interactive SQLite access to one thread, sweeps on one separate bounded connection, drains both lanes before close, and refuses work after close |
...
| `web/desktop.py` | Rebuilt | Desktop packaging | `e757a6fe`, `d0dbf737`, `c78d0c32` | Owns platform paths, one V2 product, one native WebView2 window, direct browse and Trash verbs, the native folder chooser, and idempotent shutdown; source paths passed real-window and catalog-release proof |
| `web/templates/v2.html` | Rebuilt | Desktop UI | `b153c471`, `e757a6fe`, `9ddb91a4`, `a33df983` | The semantic desktop document names library, Trash, native folder setup, browse controls, inspector, loupe, toast, and typed-count deletion without inherited templates or inline behavior |
| `web/static/v2/index.css` | Rebuilt | Desktop UI | `b153c471`, `e757a6fe`, `9ddb91a4`, `a33df983` | One neutral visual vocabulary keeps chrome subordinate to photographs, positions bounded virtual cells, makes destructive intent distinct, preserves full loupe images, and prevents hidden controls leaking through layout rules |
...
| `web/static/v2/net/index.js` | Rebuilt | Desktop UI | `b153c471`, `e757a6fe`, `a33df983` | Waits for the native bridge and exposes the sole browse and Trash vocabulary; it contains no fetch, URL, route, or HTTP concept |
| `web/static/v2/store/index.js` | Rebuilt | Desktop UI | `b153c471`, `9ddb91a4`, `a33df983` | Owns the sole mutable UI state, current lens, sparse indexed photo map, selection position, library/Trash counts, and registry |
...
| `web/static/v2/kit/page-cache.js` | Rebuilt | Desktop UI | `9ddb91a4` | One sparse page owner deduplicates in-flight reads, rejects stale sort generations, jumps to arbitrary offsets, and turns one failed page into one visible error rather than a retry storm |
| `web/static/v2/lens/library.js` | Rebuilt | Desktop UI | `b153c471`, `e757a6fe`, `9ddb91a4`, `a33df983` | One lens reconciles only the visible overscan neighborhood for Library or Trash, with view-specific empty truth and accessible selection without image rebuilds |
| `web/static/v2/shell/app.js` | Rebuilt | Desktop UI | `b153c471`, `e757a6fe`, `9ddb91a4`, `a33df983` | The coordinator owns view switching, sparse pages, direct product verbs, native folder choice, sorting, density, race-safe details, layered Escape, grid keys and loupe navigation while delegating the destructive conversation |
| `web/static/v2/shell/trash.js` | Rebuilt | Desktop UI | `a33df983` | One focused workflow owns Restore, exact Undo toast, preflight, typed-count dialog and permanent outcome copy; its native proof changed 3→2→3 without moving bytes and left the destructive submit disabled |
...
| `web/test_v2_page_cache.mjs` | Rebuilt | Desktop UI proof | `9ddb91a4` | Refutes duplicate page reads, stale-generation repaint, and permanent visible-page retry loops at arbitrary large offsets |
| `web/test_core.py` | Rebuilt | Core proof | `284ba3f2`, `da1ccd7e`, `0639ab8d`, `bce1178f`, `7227545d`, `e331be8f`, `84d832f2`, `155d450c`, `e3335a42`, `e757a6fe`, `d0dbf737`, `c78d0c32` | Adds Trash refuters for prior-state restore, stale Undo, invalid-member atomicity, 5,000-photo batches, duplicate identities, away drives, stale copy hints, changed bytes, exact counts, record-copy-last interruption and resume; the focused V2 suite passes 99 tests on Windows |
...
| `web/test_desktop.py` | Rebuilt | Desktop packaging | `e757a6fe`, `d0dbf737`, `c78d0c32` | The real direct bridge crosses attach, refresh, browse, tile, Trash page/count, Undo and destructive dry-run, then closes idempotently and releases the catalog |
