...
| `web/work.py` | Rebuilt | Work and cache | `0639ab8d`, `bce1178f`, `155d450c`, `39bc64c2`, `220ad680` | Owed work and debt are queries over explicit kinds; one owned worker does the most-owed thing for the newest photograph -- on-screen photographs first, then identity, metadata, grid, loupe for one photograph before the next -- projects ready answers, counts what it finished for the window's pulse, and releases its connection without global lifecycle state |
