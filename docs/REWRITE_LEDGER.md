...
| `web/model/intake.py` | Rebuilt | Import and synchronize | `f86ab56e`, `9a9ba1af` | Import is `photos.put` per file plus one pure destination rule (Raws/Digital, Raws/Film Scans/<Roll>, Snapshots, Edits by the photograph's own date); `scan` stages a source, `guess_kind` reads it, `bring` verifies every copy before anything else, takes the next free name on a collision and never overwrites, skips by identity what a drive already holds, clears a source only after its copy is verified and recorded, stops between files and resumes by running again; `cards` notices a DCIM volume cheaply. Refuters hold each promise |
| `web/synchronize.py` | Removed | Import and synchronize | `<deleting>` | The V1 plan/apply synchronize -- an offer of missing files the owner confirmed -- is replaced by the sweep that follows the folders, recognises moves and rewrites, and shows missing photographs with a Forget verb |
| `web/importer.py` | Removed | Import and synchronize | `<deleting>` | The V1 intake registry is replaced by `model/intake.py`'s `bring`: `put` per file by one rule |
| `web/features/catalog/synchronize.py` | Removed | Import and synchronize | `<deleting>` | The route over the V1 planner left with it |
| `web/features/imports/__init__.py` | Removed | Import and synchronize | `<deleting>` | The V1 import feature boundary left with its staging, card, film, journal, service, taxonomy and routes |
| `web/features/imports/staging.py` | Removed | Import and synchronize | `<deleting>` | 755 lines of staged-import machinery are replaced by `intake.scan` and the overlay's staged view |
| `web/features/imports/taxonomy.py` | Removed | Import and synchronize | `<deleting>` | 649 lines of destination logic are replaced by one pure `intake.destination` |
| `web/features/imports/card.py` | Removed | Import and synchronize | `<deleting>` | Stream-copy, verify, never-overwrite and clear-card live in `intake.bring` over `photos.put` |
| `web/features/imports/film.py` | Removed | Import and synchronize | `<deleting>` | Film rolls return as a kind of the one rule (Raws/Film Scans/<Roll>) when film import is built; the V1 zip/rar path is not carried |
| `web/features/imports/move_journal.py` | Removed | Import and synchronize | `<deleting>` | A move is a recognised sweep outcome, not a journal |
| `web/features/imports/routes.py` | Removed | Import and synchronize | `<deleting>` | HTTP import routes are replaced by direct bridge verbs |
| `web/features/imports/service.py` | Removed | Import and synchronize | `<deleting>` | The import batch service is replaced by `bring`'s tally and Recently added |
| `web/data/repositories/imports.py` | Removed | Import and synchronize | `<deleting>` | Import batch SQL left with the batches |
| `web/static/js/desktop/import_stage.js` | Removed | Desktop UI | `<deleting>` | The 826-line staged-import canvas is replaced by the 200-line import overlay |
| `web/test_imports.py` | Removed | Import and synchronize proof | `<deleting>` | Exercised the deleted registry |
| `web/test_import_move.py` | Removed | Import and synchronize proof | `<deleting>` | Exercised the deleted journal |
| `web/test_import_staging.py` | Removed | Import and synchronize proof | `<deleting>` | Exercised the deleted staging |
| `web/test_import_taxonomy.py` | Removed | Import and synchronize proof | `<deleting>` | Exercised the deleted taxonomy; the rule's refuters live in `test_core.py` |
| `web/test_catalog_synchronize.py` | Removed | Import and synchronize proof | `<deleting>` | Exercised the deleted planner; the away-drive invariant lives in `test_core.py` |
...
| `web/static/v2/shell/intake.js` | Rebuilt | Desktop UI | `9a9ba1af` | The import conversation: staged photographs with pictures, suspects dimmed, the kind guessed or asked, destinations shown as truth, one button, a countdown to the card being free, and what happened at the end; proven natively on a fake card with a rollover collision and an already-held photograph |
