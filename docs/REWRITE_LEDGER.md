...
| `web/model/copies.py` | Rebuilt | Core | `96e985d9`, `7227545d`, this change | A complete marker-stable sweep admits observed photos in bounded committed batches so the grid can fill, reports changed bytes without replacing identity, and retires stale hints only after the complete marked drive stays present |
...
| `web/boot.py` | Rebuilt | Verification and fresh boot | `e331be8f`, `155d450c`, `e697d24e`, this change | One `Library` owns the product verbs; `OwnedLibrary` confines interactive SQLite access to one thread, sweeps on one separate bounded connection, drains both lanes before close, and refuses work after close |
...
| `web/routes/library.py` | Rebuilt | Browse, disk, and tiles | `e697d24e`, `a3bbd3cd`, this change | Typed V2 routes serve drives, refresh, bounded photo pages, embedded details, and content-keyed JPEG tiles; choosing a folder is immediate and its refresh cannot block reads |
| `web/routes/system.py` | Rebuilt | Desktop UI transport | `5c3d24eb` | Local health, route truth, and prepare-quit are three read-time answers; prepare-quit closes the owned product before acknowledging the desktop shell |
| `web/v2_app.py` | Rebuilt | Verification and fresh boot | `5c3d24eb` | One lifespan constructs and starts the isolated V2 product, mounts only V2 routers, exposes no server documentation surface, and deterministically closes on shutdown |
| `web/test_core.py` | Rebuilt | Core proof | `284ba3f2`, `da1ccd7e`, `0639ab8d`, `bce1178f`, `7227545d`, `d33c655e`, `e331be8f`, `84d832f2`, `155d450c`, this change | Proves the fresh `Library`, progressive sweep visibility, native EXIF, display orientation, metadata projection/repair, decision override, pagination, decode limits, and persisted tiles; 81 tests and 4 subtests pass on Windows |
| `web/test_v2_routes.py` | Rebuilt | Desktop UI transport proof | `e697d24e`, `a3bbd3cd`, this change | Refutes malformed-scope widening, index loss, cross-thread SQLite access, cancelled-close handle leaks, scan-blocked browsing, invalid tile recipes, and fake transport data; 8 tests and 6 subtests pass against a real fresh V2 catalog |
| `web/test_v2_app.py` | Rebuilt | Verification and fresh boot | `5c3d24eb` | Proves the default V2 catalog/cache cannot collide with inherited state, the real route graph mounts, prepare-quit returns only after handle release, and post-close health is unavailable |
