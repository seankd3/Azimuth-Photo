...
| `web/work.py` | Rebuilt | Work and cache | `0639ab8d`, `bce1178f`, `155d450c`, `39bc64c2` | Owed work and debt are queries over explicit kinds; one owned worker projects ready answers without a second scanner, releases its connection without global lifecycle state, and counts what it finished so a window can ask for one integer instead of a query |
...
| `web/boot.py` | Rebuilt | Verification and fresh boot | `e331be8f`, `155d450c`, `e697d24e`, `e3335a42`, `b153c471`, `e757a6fe`, `d0dbf737`, `c78d0c32`, `8c3f24a1`, `39bc64c2` | One `Library` owns browse, tile, cull, Trash, restore, Undo, empty and pulse verbs; `OwnedLibrary` confines interactive SQLite access to one thread, sweeps on one separate bounded connection, drains both lanes before close, and refuses work after close |
...
| `web/desktop.py` | Rebuilt | Desktop packaging | `e757a6fe`, `d0dbf737`, `c78d0c32`, `8c3f24a1`, `39bc64c2` | Owns platform paths, one V2 product, one native WebView2 window, direct browse and Trash verbs, the native folder chooser, and idempotent shutdown; source paths passed real-window and catalog-release proof |
...
| `web/static/v2/net/index.js` | Rebuilt | Desktop UI | `b153c471`, `e757a6fe`, `a33df983`, `8c3f24a1`, `39bc64c2` | Waits for the native bridge and exposes the sole browse and Trash vocabulary; it contains no fetch, URL, route, or HTTP concept |
...
| `web/static/v2/shell/app.js` | Rebuilt | Desktop UI | `b153c471`, `e757a6fe`, `9ddb91a4`, `a33df983`, `d2a8357e`, `39bc64c2` | The coordinator owns view switching, sparse pages, direct product verbs, native folder choice, sorting, density, race-safe details, layered Escape, grid keys, loupe navigation and the P/U/X cull keys; it follows the library through one in-place refresh shared by the sweep loop and the worker's pulse, and renders the status line from state (a transient notice over scanning or *Reading N photos…*) as its one writer |
