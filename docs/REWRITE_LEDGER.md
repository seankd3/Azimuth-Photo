...
| Core model and schema | Designing | Photos, copies, drives, decisions, sets, scope, and cache are the whole durable model | A fresh five-table catalog opens through `model.connect`; decisions own provenance, drive reads are explicit, imports become photo and copy facts atomically, identities cover every byte, moves are collision-proof, alternate copy tails survive sweeps, and cache kinds are explicit values. Sets, scope, and real desktop integration remain before the area is Proven |
...
| `web/model/photos.py` | Rebuilt | Core | `b12f4ff8`, `ca40451c`, `96e985d9`, this change | Owns admission, full-byte identity, truthful open/state, collision-proof put/move, and version grouping; a verified import now records its photo and copy in the same transaction |
| `web/model/copies.py` | Rebuilt | Core | `96e985d9`, this change | A complete marker-stable sweep admits new photos, reports changed bytes without replacing identity, retires only hints, and recognizes explicit alternate copy tails instead of duplicating photos |
| `web/model/backup.py` | Rebuilt | Keep and safety | `b12f4ff8`, this change | Backup only adds byte-proven regular files; reclaim ignores symlinks, honors alternate tails, repeats byte proof at the destructive gate, and removes at most one expendable copy per call |
...
| `web/test_core.py` | Rebuilt | Core proof | `284ba3f2`, `da1ccd7e`, `0639ab8d`, `bce1178f`, this change | Opens an empty catalog and proves a real JPEG travels from fresh drive sweep through browse, full identity, and a 400 px persisted tile; import/copy/backup safety refusals are included; 66 tests and 4 subtests pass on Windows, but real desktop proof is still owed |
