...
`memory_pressure` 615, `model_pool` 618, `work_coordination` 341,
`bulk_scheduler` 187, `propagation_queue` 108, `hdd_governor` 84 — **2,397 lines,
verified file by file** — plus 35 start-a-job or ask-a-status routes.
...
| **`scope`** *(one filter language)* | `scope(conn, text: str) -> (sql, params)`  — raises `ValueError` on anything it cannot read | Makes a folder, a collection uuid, a keyword, a saved view, a smart query, a star floor, a search result and an export selection **the same argument**. `library.photos()` gains `scope=` and loses `folder=` / `starred=`. Deletes `collections/smart.py` 311 (eight resolvers for one question), `library/saved_views.py` 139 (table holds 0 rows), `collections/graph.py` 254 (`collection_links` 0 rows), `core/query_constraints.py` 432, `repositories/filter_options.py` 95, and the seven containment guards. | ~1,350 |
...
there is nothing disposable left to store. A `sets.py` module is unnecessary: a
named set is `decisions.current(family, glob=)` over a uuid, which is `decide()`
already doing its job. **Adding a parameter to a correct function deletes more
than adding a primitive, and leaves the concept count unchanged.**
...
| **3** | Library, and the seam pivot. **`GET /api/photos?scope=` ships before a single old route is removed** | `collections/smart.py`, `library/saved_views.py`, `collections/graph.py`, `core/query_constraints.py`, `repositories/filter_options.py`. If the UI cannot be moved onto `scope`, `routes/`'s budget is wrong and the UI is not |
