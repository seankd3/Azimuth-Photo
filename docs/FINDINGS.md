...
| PN4 | A sweep the library knows cannot be seen whole | the preview: the Panorama fact is a control that merges the sweep from its loupe tiles on the sweep lane (OpenCV's stitcher, the black canvas trimmed on an eighth-scale mask; his Mt Washington sweep in 3.4 s, 7,337 × 2,163 from 4,096 px tiles), kept as one file on the run and shown as a strip under the facts and, pressed, in the loupe as a preview; a stitcher that cannot place the frames is remembered as failed and the fact stays a fact. Not evicted (not among the worker's kinds): a few megabytes per sweep, and sweeps are rare | shipped |
