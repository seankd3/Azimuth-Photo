...
| CR3 | "Empty trash button is very laggy and takes forever for the dialog box to pop up": the dialog waited on a dry run of the deletion that read every trashed file byte for byte (hashing each anchor, comparing each copy, probing each drive's canonical address by hash) before it would open; on a Trash of hundreds of RAWs that is many seconds on the interactive lane, and the deletion then read them all again | the dialog opens at once with the count the window already knows; the preflight runs while the person reads and only checks that every drive is attached and every file is where it was filed, and the button arms when it answers (an away drive is the dialog's error, not a toast before it); the deletion itself still verifies every byte. Harness, preflight of 1.2 s: 1,209 ms to open before, 5 ms after, the button disarmed until ready | shipped |


## Sniffing friction (2026-09-12)
