...
2. **Stars: two distinct fields, never merged.**
   ...
   - `elo_stars` (new, computed): hub projects Elo percentile → 0–5 stars (thresholds configurable; default 5★=top 2%, 4★=next 8%, 3★=next 20%, else unstarred; only photos with ≥N comparisons project — predicted-only Elo does not project). Plugin writes elo_stars INTO LR only when the photo's current LR stars == the plugin's last projection (i.e. user hasn't overridden). User-set stars always win locally and flow back as lr_rating. The projection is recomputed hub-side, delivered as part of the flag/rating delta stream.
