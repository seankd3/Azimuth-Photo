...
**Two more "mutate disk before the DB commit gate" siblings (2026-07-16, destructive-ordering sweep → Codex sol fix + Fable review)** — found by auditing the empty-trash bugs class:
- **HIGH — taxonomy reclassify stranded keep-worthy originals.** `features/imports/taxonomy.py::reclassify_misplaced_personal_photos` `shutil.move`d every matched file then did one `conn.commit()` after the whole loop; a commit failure/crash mid-batch rolled back ALL filepath UPDATEs while every file was already moved → catalog points at missing originals, and (unlike trash) NO auto-heal. A move cannot be atomic with the DB, so fix = commit per row (move→UPDATE→commit each), bounding the blast radius from the whole batch to one in-flight file; on per-row commit failure it records the error + stops with prior rows durable; re-run heals the one stranded row. Regression test proven fails-on-old / passes-on-new. (95b4293f)
- **MEDIUM — publish revoke was not retryable.** `features/publish/deployer.py::_revoke_sync` `shutil.rmtree`d the live gallery before `persist_revoke`; a failure between them left the gallery gone but catalog still published, and retry hit rmtree on a missing path → `PublishDeployError` "still published" → stuck. Fix: tolerate `FileNotFoundError` (already-removed = success) so a retry completes; genuine removal errors still raise. Regenerable output so no data loss, but unblocks the stuck state. (9b849318)
- Sweep verdict: the rest of the destructive surface (trash move/restore, import copy/clear-card, hub upload finalize, develop save/HDR/pano register, XMP write-back, prefetch reclaim, rescan/mark-missing) is SAFE for this antipattern (commit-first or orphan-avoidance ordering).

**"Dishonest success" — UI reported trash/restore/develop as done when it partially or fully failed (2026-07-16, silent-failure sweep → Codex fix + Fable review)**
- The trash-move/restore endpoints return HTTP 200 with a partial-success body (`trashed`/`restored` ids + `errors`), but the desktop callers checked only `result.ok` and toasted the REQUESTED count. Fixed to read the ACTUAL confirmed ids: `trash.js` (`trashSelectedImages`/`restoreSelectedTrash`) and `duplicates.js` (`keepCoverForStack`/`keepCoversEverywhere`) now toast the real count + a partial "Moved N of M — K couldn't be moved" when errors exist, keep failed items selected for retry, target undo at only the confirmed ids, and (duplicates) only collapse a stack on FULLY-confirmed trash — a partial/failed cull leaves the stack visible. Brings these in line with the app's existing actual-count convention (empty-trash/import/sync-paste). 
- Develop autosave (`develop.js scheduleSave`) previously ended `.catch(() => {})` — a failed PUT was silent, so the canvas showed edits that never persisted and vanished on reopen. Now a failed save sets `entry.unsaved`, shows one debounced "Couldn't save your edits — check your connection" toast per unresolved failure (cleared on a later confirmed save), and keeps the canvas edit intact (no destructive rollback). A `saveVersion` guard prevents a stale in-flight save from clobbering a newer edit's state. (8b94eef5; frontend live)

**RESOLVED: the "flaky" empty-trash Playwright test is a TEST-INFRA artifact, not a prod bug (2026-07-16, traced not guessed)**
- `test_empty_trash_then_all_photos_loads_promptly` fails consistently (not flaky) with `assert 0 == 12` — the live test server returns `deleted_count=0`. Investigation: (1) the product `empty_trash` is CORRECT — a direct call on satellite-seeded data returns 12/12, zero errors; (2) the live failure is SQLite write-lock contention: something holds the writer lock ~2.6s during the test's empty-trash, so the delete's 0.7s busy-retry budget gives up and soft-fails to 0; (3) traced (connection-layer slow-write logger) the holder = `PRAGMA wal_checkpoint(TRUNCATE)` at 2.28s, from `_checkpoint_temp_wal_async` in data/connection.py, which runs on EVERY connection close but ONLY for EPHEMERAL DBs (`is_ephemeral_db_path` = under the system temp dir). The 139k-row seed builds a huge WAL; truncate-checkpoint-on-close self-contends with the test's empty-trash. **Prod DBs are never ephemeral → this checkpoint never runs in prod → empty-trash is unaffected in production.** Earlier "prod robustness bug / satellite" framing was wrong; corrected by tracing. No prod fix needed. **FIXED (8c048c74):** the ephemeral WAL checkpoint now uses PASSIVE (non-blocking) instead of TRUNCATE, so a large ephemeral seed no longer self-contends — full suite green (945 passed). Ephemeral-only path, zero prod impact.
- **Real prod win found alongside (SHIPPED 5b9e672c):** `_backfill_shoot_hints` did a single `executemany` over the whole catalog — a first-time/version-bump backfill on Sean's 139k+ library would hold the writer lock ~2.6s, starving interactive writes. Now commits in 2000-row chunks (systems-factor #2). Dormant on already-backfilled prod; deploys on next restart.
