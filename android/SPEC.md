...
E3. **Themed icon + splash.** Add `androidx.core:core-splashscreen`, Theme.AzimuthPhoto.Splash with Ink background + the aperture vector, postSplashScreenTheme → current theme. Manifest application uses splash theme on MainActivity. The adaptive icon already has a monochrome layer — verify and keep.
