...
- `open` "Real stars alongside Elo" — 07-31 · **two independent axes, neither writing the other.** Stars are *asserted* — the user's explicit verdict, authoritative, and what exports to Lightroom verbatim. Elo is *earned* — evidence accumulated from pairwise comparisons, and it drives Refine, smart ordering and propagation. Requires a real `rating` column, plus facet, sort and write UI on all three clients; the mobile rating sheet that currently writes where nothing can read is the first thing this fixes. **Hard constraint:** setting a star must never mutate earned Elo, and Elo must never silently write a star — same law as the taste/Elo blend. For Lightroom export, an explicit star wins; where none exists the existing Elo projection (`elo_stars.py:86`) fills in and is marked inferred, so a projection is never mistaken for the user's verdict.
