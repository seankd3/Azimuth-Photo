...
- **Current evidence:** the current paused head collects 1,425 tests through
  ...
  closed in full order. Deterministic `BUG-PREGEN-WATCHDOG-01` was then traced
  to exact `main`: the worker recorded `last_progress_at` on decode submission
  and completion, while the watchdog ignored that heartbeat and considered
  only the older successful-write/start timestamps. `develop` predates this
  watchdog contract. Commit `7ad2d5f3` includes the existing heartbeat in the
  stall anchor without changing reset behavior for a truly stuck batch. The
  exact progressing-batch test moved from one failure with two false executor
  resets to 10/10 passes; the real-stall pair passed 2 tests, the related
  preview/pregeneration matrix passed 62 tests, and Ruff plus quick are green.
  The frozen-source complete checkpoint on `ba0fffe4` then finished without a
  native abort: 14 failed, 1,524 passed, 3 skipped, 2 deselected, and 399
  passing subtests in 476.98 seconds. The exact set is the stable 18 identities
  minus the repaired watchdog test and the three known variable performance
  budgets; no new identity appeared. `BUG-RUNTIME-02` was then reproduced as an
  import-order escape carried by both parent baselines: direct AI-mask import
  selected a disposable custom home, but HDR redundantly ran the app-level
  environment initializer, backfilled an inherited host Develop default, and
  sent HDR, panorama, and RAW caches outside that home. Commit `e8265d81`
  removes only HDR's redundant initializer; `app.py` remains the application
  initializer. The selected-home and explicit-cache-root contracts passed 2
  tests; the runtime/path/HDR/panorama/compatibility matrix passed 46 with 1
  platform skip; Ruff and quick are green. The frozen-source complete checkpoint
  on `e6296737` then finished without a native abort: 13 failed, 1,526 passed, 3
  skipped, 2 deselected, and 399 passing subtests in 489.59 seconds. The exact
  set is the latest 14 identities minus the repaired Develop-root test; no new
  identity appeared. Relative to the stable 18, the repaired watchdog and
  Develop-root identities plus the same three variable performance budgets are
  absent. A bounded read-only triage on frozen source `ea258ce68` then ran the
  three budgets in 10 fresh grouped processes plus five fresh isolated
  processes per node, always using the same synthetic 2,000-image catalog,
  three cache-cleared samples, Python 3.13.5, SQLite 3.49.1, and tmpfs-backed
  temporary roots. Rankings first page and the visible-library query passed
  15/15 process assertions; date histogram passed 13/15 and missed only in two
  grouped processes at 83.1 and 93.3 ms against its 80 ms budget. The test
  process reported `nproc=3` while host load ranged from 3.74 to 5.57 and about
  11 GiB of swap was occupied. The same three identities have also appeared or
  disappeared together across frozen full gates. Their tests and relevant
  Library service/route/repository paths are byte-for-byte production `main`,
  so this is not evidence of a reconciliation regression. Release owns a
  contention-controlled gate method under `BUG-PERF-BUDGET-01`; Core Library &
  Safety owns query optimization only if that method proves a product-path
  miss. `BUG-TEST-SHIM-01` was then isolated to Phase 1's compatibility
  boundary: exact `main` and pre-fix integration fail the living-shell contract
  because it reads the deprecated wrapper, while exact `develop` passes because
  that path still contains the implementation. The executable probe also found
  the canonical script still used CommonJS imports under the repository's ES
  module declaration, so both command names exited before printing help.
  Commit `6ccac86bd` makes the Azimuth script, check runner, opt-in test, and
  developer docs canonical; retains the prior filename only as the explicit
  forwarding shim; and converts only the smoke script's five built-in imports.
  Three exact contracts, all 16 UI contracts, command/help equivalence, syntax,
  Ruff, and quick are green. `BUG-SHORTCUT-01` was then classified as a stale
  test inherited from `develop`, not a product regression: exact `main` passes
  its older Events/Y contract, while exact `develop` and pre-fix integration
  fail after deliberate commit `9cb15beb5` replaced Events/Y with the primary
  Collections/C lens. The current sheet, C/O/M/H handlers, visible tooltips,
  ...
  changed during the audit. `BUG-TEST-02A` was then closed at the test-harness
  boundary in `ade6c4948`: an opt-in mixin gives only `PeopleTests` and
  `ThumbnailBulkWarmupTests` a deterministic 1,024-byte pressure reading and a
  safely above-resume host-available reading, then resets all injected state.
  Production watermarks and pause behavior are unchanged. A focused contract
  first proved an above-hard production reading still pauses People and
  thumbnail work before and after isolation; the eight affected identities plus
  that proof passed 9 tests, the memory-pressure/People/thumbnail-bulk matrix
  passed 85, Ruff and quick are green. Two frozen-source complete
  non-Playwright gates then produced the identical result: 3 failed, 1,539
  passed, 3 skipped, 2 deselected, and 399 passing subtests, in 505.80 and
  447.50 seconds. The only failures in both gates are the three untouched
  `BUG-TEST-02B` scheduling identities; all eight memory-gated identities and
  all three variable performance budgets were absent.
...
| 2026-07-25 BUG-PREGEN-WATCHDOG-01 on `7ad2d5f3` | Preview stall-watchdog truth | Before: exact node failed 1/1 and reset the executor twice while a decode heartbeat advanced every 50 ms. After: exact node passed 10/10 with zero false resets; the true-stall/progress pair passed 2 tests; related preview/pregeneration coverage passed 62 tests; Ruff and quick green. Complete checkpoint on `ba0fffe4`: 14 failed, 1,524 passed, 3 skipped, 2 deselected, 399 subtests passed in 476.98 s; watchdog identity closed, no new identity, and only three known variable performance budgets absent from the prior 18 | Preserve true-stall recovery and prove one real workload longer than the five-minute production stall window | `done awaiting integration` |
| 2026-07-25 BUG-RUNTIME-02 on `e8265d81` | Develop cache-root containment | Before: controlled inherited-default reproduction failed 1/1; AI masks selected the custom root while HDR, panorama, and RAW escaped to `/home/sean/.cache/photoarchive/develop`. After: selected-home and explicit configured-root contracts passed 2 tests; related runtime/path/HDR/panorama/rebrand matrix passed 46 with 1 platform skip; Ruff and quick green. Complete checkpoint on `e6296737`: 13 failed, 1,526 passed, 3 skipped, 2 deselected, 399 subtests passed in 489.59 s; Develop-root identity closed, no new identity, and the same three variable performance budgets absent | Preserve valid configured roots; packaged Windows/macOS child-process import order remains unverified | `done awaiting integration` |
...
| 2026-07-25 BUG-TEST-02A on `ade6c4948` | Host-independent People/thumbnail bulk contracts | Exact eight affected nodes plus production-gate/isolation proof: 9 passed. Memory-pressure/People/thumbnail-bulk matrix: 85 passed. Ruff and quick green. Complete gate 1: 3 failed, 1,539 passed, 3 skipped, 2 deselected, 399 subtests passed in 505.80 s. Gate 2: identical counts in 447.50 s. Both failure sets contain only caption cancellation, first-run preview, and decode-budget cancellation from BUG-TEST-02B; all eight BUG-TEST-02A identities and all three variable performance budgets are absent | Preserve opt-in test scope and the above-hard production pause proof; do not route production policy through the test helper or broaden this into BUG-TEST-02B | `done awaiting integration` |
...
| BUG-TEST-02A | `done awaiting integration` / P0 | Release | The eight People/thumbnail bulk contracts were host-dependent because they exercised production memory gates against live cgroup/host state despite asserting non-policy behavior. Commit `ade6c4948` adds one opt-in test mixin that injects a deterministic below-watermark reading only for `PeopleTests` and `ThumbnailBulkWarmupTests`, with reset cleanup. Production watermarks and gating are unchanged; an above-hard reading still pauses both real decision paths before and after isolation. Focused 9 passed, related matrix 85 passed, Ruff/quick green, and two complete gates closed all eight identities with no new or variable-performance failure | Preserve the narrow opt-in scope and above-hard production-gate regression. Any product memory-policy change remains separately owned and must not use the test helper |
...
| BUG-PREGEN-WATCHDOG-01 | `done awaiting integration` / P0 | Core | Exact `main` recorded `last_progress_at` on pregen decode submission/completion but the watchdog ignored it and anchored only to older successful-write/start times; `develop` predates the contract. Commit `7ad2d5f3` takes the newest generation, heartbeat, or start timestamp, so active legitimate work no longer resets its executor while the no-heartbeat fixture still recovers. Before: exact node failed 1/1 with two resets. After: exact node 10/10, watchdog pair 2 passed, related preview/pregeneration matrix 62 passed, Ruff and quick green. The complete checkpoint on `ba0fffe4` closed the identity with no new failure and no native abort; its 14 failures are the stable 18 minus this repair and three known variable performance budgets | Preserve full-order closure. A real RAW workload longer than the five-minute production window and heartbeat behavior during a genuinely wedged native decode remain unverified |
| BUG-RUNTIME-02 | `done awaiting integration` / P0 | Core | Both parent baselines let `features.develop.hdr` redundantly call the application environment initializer during import. In a child/custom-home flow, AI masks first selected the requested root, then HDR backfilled an inherited host Develop default and caused HDR, panorama, and RAW to escape to `/home/sean/.cache/photoarchive/develop`. Commit `e8265d81` leaves `app.py` as the sole app initializer and adds deterministic selected-home plus explicit configured-root contracts. Before: controlled node failed 1/1. After: focused 2 passed; runtime/path/HDR/panorama/rebrand matrix 46 passed with 1 platform skip; Ruff and quick green. The complete checkpoint on `e6296737` closed the identity with no new failure and no native abort; its 13 failures are the latest 14 minus this repair | Preserve full-order closure. Packaged Windows/macOS child-process import order remains unverified; broader env-name migration stays separately decision-gated |
| BUG-PERF-BUDGET-01 | `unverified` / P1 | Release | The rankings-first-page, date-histogram, and visible-library synthetic budgets co-vary across complete gates despite frozen source: all three failed at 201.9/94.3/195.6 ms in one gate, then all three were absent from three later checkpoints. On `ea258ce68`, rankings and visible-library passed 15/15 fresh-process assertions; histogram passed 13/15 and missed only in two grouped processes. Relevant code is byte-for-byte `main`; current evidence classifies the full-gate identities as host-contention/budget-method variance, not a reconciliation regression. Product-path ownership, if a controlled miss is proved, is Core Library & Safety: rankings and visible-library share `api_rankings_impl` plus the rankings repository, while histogram uses `date_histogram_payload` plus its cached repository query | Release must establish an idle-host and declared-load matrix with fixed catalog, fresh processes, correctness checks, p50/p95/p99/worst, and enough repetitions to state a failure rate. Do not loosen 200/80/125 ms budgets or optimize queries until that matrix separates harness noise from a repeatable Core miss |
| BUG-SHORTCUT-01 | `done awaiting integration` / P1 | Desktop | Commit `9cb15beb5` intentionally made Collections a primary lens and replaced Events/Y with Collections/C across the sheet, keyboard handlers, top switcher, architecture doctrine, and Collections-lens contract. Exact `main` passes its older internally consistent Events/Y test; exact `develop` and pre-fix integration fail only because `test_shortcut_sheet.py` retained that older key map. Test-only commit `7d71b91b6` expects C/O/M/H, pins every key to the matching lens call, and checks the visible tooltip plus accessible lens name. Two focused tests, Ruff, and quick are green; product code is unchanged | Preserve the intentional C/O/M/H mapping. Static contracts do not prove event dispatch, focus movement, or screen-reader announcement in a running client; include those four keys in DESK-04's later Windows/Linux keyboard and accessibility journey |
| BUG-TEST-SHIM-01 | `done awaiting integration` / P1 | Release | Phase 1 moved the implementation to `azimuth-browser-smoke` and replaced the prior path with a wrapper, but the living-shell contract, opt-in browser test, check runner, and developer docs still selected the wrapper. Exact integration/main failed the contract; exact develop passed. A direct executable probe found the canonical copy also retained CommonJS `require()` under a package declared as ES modules, so both command names exited 1. Commit `6ccac86bd` points every canonical caller and customer-facing usage at Azimuth, preserves the old filename only as an exact forwarding shim, and makes the implementation executable with ES imports. Three exact contracts and all 16 UI contracts pass; canonical/shim help is identical and Azimuth-only; syntax, Ruff, and quick are green | Preserve the shim only through the decision-gated compatibility window. The real Chrome/CDP smoke against a disposable server was not run because this slice could not touch services/runtime; require it before release rather than treating static shell coverage as browser proof |
...
| BUG-TEST-02A | `sprint-integration` / `ade6c4948` | Opt-in deterministic memory readers for People and thumbnail-bulk contract classes; production gating remains real and separately proved | Focused 9 passed; related matrix 85 passed; Ruff/quick green; two complete gates closed all eight identities and retained only BUG-TEST-02B |
| BUG-TEST-02B | `sprint-integration` / `c17879607` (source-frozen audit) | Executor/event-loop timing boundary for caption cancellation, first-use preview, and decode-budget cancellation | Evidence only; all three remain active and reproduced identically in both complete gates after BUG-TEST-02A |
