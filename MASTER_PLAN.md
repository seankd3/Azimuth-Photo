...
- `partial` "as you work overnight I want you to continously idenitify the limiting factor of large work, find the slowest part of the app the bottleneck and agressively optomize it, until something else is the bottle neck, and then do the same for it. repeat this process over and over and over" — 07-20 · `scripts/perfbench.py:1`, `scripts/bench.py:1`, `web/perf/baseline.js` — Bottleneck measurement tooling exists (perfbench, standing bench, baseline); missing is any continuous overnight identify→optimize→repeat automation (no perf cron/timer/CI loop found under scripts/,…
...
- `partial` "SSd space, ram, cpu, gpu, all of it keep track of benchmarks over time and log them in the commits as you go, I want to be able to really track progress and watch as the app gets faster and faster" — 07-20 · `web/perf/baseline.js`, `scripts/bench.py:18`, `scripts/perfbench.py:37` — One committed baseline snapshot and tools that tag git SHA exist; time-series history is written outside the checkout (runtime state dir / AZIMUTH_PERFBENCH_HISTORY), not logged into commits as continuous…
