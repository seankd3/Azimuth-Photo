...
- `open` "Stars should be applied by the photos elo" — 07-31 · **stars are Elo's readable face, not a second signal.** One quality signal earned from pairwise comparison; the star is its projection, computed and stored so it can be filtered, sorted and exported to Lightroom as a real value. This supersedes the independent-axes design above — there is no competing user-asserted score, which keeps the propagation system on the single signal it was built for.
  - Applying Sean's standing "diagnosis, not question" doctrine to the override case, which he did not specify: a manually set star is a **remembered correction**, not a value Elo may overwrite. Setting one pins that photo and marks it user-set; Elo keeps earning underneath but stops writing that photo's star until released. So the app always shows a conclusion rather than an empty rating, and the one time the user disagrees, the disagreement sticks. **Flagged as inference — correct it if the override should instead just nudge Elo and let it recompute.**
