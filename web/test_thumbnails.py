...
class ThumbnailMaintenanceFacadeTests(unittest.TestCase):
    ...
    def test_cache_safety_helper_owns_marker_write_and_facade_matches(self):
        ...
        self.assertTrue(os.path.exists(os.path.join(thumbnails.SSD_CACHE_DIR, thumbnails.CACHE_MARKER)))
...
class ThumbnailBulkWarmupTests(BulkMemoryIsolatedTestCase, unittest.TestCase):
    def setUp(self):
        ...
        self.old_data_providers = {
            ...
            for name in (
                ...
                "mark_image_missing_sync",
                "invalidate_cached_image_ids_cache",
                "note_cached_image_ids_added",
            )
        }
        ...
        # data_providers forwards straight to db now; there is no configure step,
        # so the stubs go onto the module the package actually calls. tearDown
        # already restores these from self.old_data_providers.
        for name, replacement in (
            ("get_db", get_db),
            ("batch_set_orientations", batch_set_orientations),
            ("mark_image_missing_sync", mark_image_missing_sync),
            ("invalidate_cached_image_ids_cache",
             lambda **kwargs: self.provider_events.append(("invalidate", kwargs))),
            ("note_cached_image_ids_added",
             lambda cache_root, size, image_ids: self.provider_events.append(
                 ("note", cache_root, size, list(image_ids)))),
        ):
            setattr(thumbnails.data_providers, name, replacement)
        ...
        preview_priority._priority_scopes.clear()
    ...
    def test_clear_cache_refuses_unmarked_directory_with_non_cache_files(self):
        ...
        marker = os.path.join(thumbnails.SSD_CACHE_DIR, thumbnails.CACHE_MARKER)
...
class PreviewWorkTruthTests(unittest.TestCase):
    def test_work_state_distinguishes_active_blocked_idle_and_stalled(self):
        ...
        self.assertEqual(
            thumbnail_status.work_state(
                {**active, "last_generated_at": 1.0, "last_progress_at": 999.0},
                preview_remaining=4,
