...
class SettingsRouteTests(BackendTestCase):
    ...
    async def test_get_rating_returns_stored_star_projection(self):
        source = await self._source()
        image_id = await self._image(source["id"], "projected.jpg")
        empty = await self._request("GET", f"/api/image/{image_id}/rating")
        self.assertEqual(empty.status_code, 200, empty.text)
        self.assertEqual(empty.json()["stars"], 0)
        # Make the image projectable and persist the projection — a lone
        # eligible photo is the top of its pack, so it projects 5 stars.
        conn = await db.get_db()
        try:
            await conn.execute(
                "UPDATE images SET elo = 1600, comparisons = 5, "
                "content_hash = 'aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa' WHERE id = ?",
                (image_id,),
            )
            await conn.commit()
        finally:
    ...
    async def test_manual_star_writes_are_retired(self):
        # Stars are computed from Elo; there is no manual star-setting gesture.
        source = await self._source()
        image_id = await self._image(source["id"], "no-write.jpg")
