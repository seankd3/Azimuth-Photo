...
export function directoryRootsHtml(roots = []) {
    return (roots || []).map((root) =>
        `<button class="bar-btn" type="button" data-action="browse-directory" data-path="${escapeHtml(root.path)}">${escapeHtml(root.label)}</button>`
    ).join('');
}
...
export function directoryBrowserListHtml({ path = '/', entries = [] } = {}) {
    ...
    const currentRow = `
            <div class="directory-tree-row current">
                ...
                <button class="bar-btn" type="button" data-action="use-browsed-directory">Use</button>
            </div>
        `;
    const childRows = rows.map((entry) => `
            <div class="directory-tree-row child ${entry.readable ? '' : 'restricted'}">
                <button class="directory-tree-open" type="button" data-action="browse-directory" data-path="${escapeHtml(entry.path)}" title="${escapeHtml(entry.path)}">
                    <span class="tree-branch"></span>
                    <span class="tree-expander">+</span>
                    <span class="tree-folder">${escapeHtml(entry.name)}</span>
                    <small>${entry.readable ? 'Folder' : 'Restricted'}</small>
                </button>
                <button class="bar-btn" type="button" data-action="select-browsed-directory" data-path="${escapeHtml(entry.path)}">Use</button>
            </div>
        `).join('');
...
export function directoryCrumbsHtml(path) {
    ...
    const crumbs = [
        `<button type="button" data-action="browse-directory" data-path="/">/</button>`,
    ];
    let current = '';
    for (const part of parts) {
        current += `/${part}`;
        crumbs.push(
            `<button type="button" data-action="browse-directory" data-path="${escapeHtml(current)}">${escapeHtml(part)}</button>`
        );
    }
