...
export async function saveSettings({
    ...
    try {
        ...
        const data = await responseDataOrError(res, 'Save failed');
...
export function resetSettings({
    ...
    showConfirmModal('Reset to defaults?', 'All settings will be restored to their default values.', async () => {
        ...
        try {
            ...
            const data = await responseDataOrError(res, 'Reset failed');
            populateSettingsForm(data.settings || {});
            rememberThumbnailOutput(data.settings || {});
            updateThumbnailChangeNotice();
            renderSettingsMeta(data);
...
export function clearThumbnailCache({
    ...
    showConfirmModal('Clear thumbnail cache?', 'This will delete all cached thumbnails. Regeneration may take hours for large archives.', async () => {
        ...
        try {
            ...
            const data = await responseDataOrError(res, 'Cache clear failed');
            renderSettingsMeta(data);
            setSettingsStatus(
...
export async function startCachePregeneration({
    ...
    try {
        ...
        const data = await responseDataOrError(res, 'Could not start pre-generation');
...
export async function stopCachePregeneration({
    ...
    try {
        ...
        const data = await responseDataOrError(res, 'Could not pause pre-generation');
        renderCacheSettingsStatus(data.cache);
...
export async function pauseEmbeddings({
    ...
    try {
        ...
        const data = await responseDataOrError(res, 'Could not pause embeddings');
...
export async function resumeEmbeddings({
    ...
    try {
        ...
        const data = await responseDataOrError(res, 'Could not resume embeddings');
        renderAISettingsStatus(data.ai_status);
...
export async function installAIModel(role = 'active', {
    ...
    const installRole = String(role || 'active');
    const label = 'selected model';
    setSettingsStatus(
        `Saving settings and starting ${label} install…`,
        'muted'
    );
    try {
        ...
        const saveData = await responseDataOrError(saveRes, 'Could not save settings');
        ...
        const installData = await responseDataOrError(installRes, 'Install could not be started');
        renderModelStatus(installData.model_status);
        renderAISettingsStatus(installData.ai_status);
        setSettingsStatus(
            installData.already_installed
                ? `${label} is already installed.`
                : `${label} install started. The worker will pick it up automatically when the download finishes.`,
            'success'
        );
        return installData;
    } catch (err) {
