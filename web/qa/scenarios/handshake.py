...
def handshake_skew(qa) -> None:
    ...
    qa.mark("queue scoped Empty Trash before any old-hub forward")
    response = qa.page.request.post(f"{qa.base_url}/api/trash/empty")
    assert response.status == 200, f"scoped Empty Trash should stay local-first: {response.status} {response.text()}"
    payload = response.json()
    assert "older version" in payload.get("hub_error", ""), payload
    expected_pending = int(qa.manifest["trash_mirror_images"]) + 1
    assert payload.get("hub_pending") == expected_pending, payload
