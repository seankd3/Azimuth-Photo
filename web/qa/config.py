...
SCRATCH_ROOT = Path(os.environ.get("AZIMUTH_QA_SCRATCH", DEFAULT_SCRATCH_ROOT)).resolve()
FIXTURE_HOME = SCRATCH_ROOT / "fixture-home"
CATALOG_DB = FIXTURE_HOME / "data" / "catalog" / "azimuth.db"
PRISTINE_DB = SCRATCH_ROOT / "azimuth.pristine.db"
...
ACTIVE_IMAGE_COUNT = int(os.environ.get("AZIMUTH_QA_ACTIVE_IMAGE_COUNT", "4003"))
if ACTIVE_IMAGE_COUNT < 4_003:
    raise ValueError("AZIMUTH_QA_ACTIVE_IMAGE_COUNT must be at least 4003")

VISIBLE_IMAGE_COUNT = ACTIVE_IMAGE_COUNT - 4
FIXTURE_VERSION = f"desktop-qa-v22-{ACTIVE_IMAGE_COUNT}"
TRASH_IMAGE_COUNT = 6
...
DEFAULT_ACTION_TIMEOUT_MS = scaled_timeout_ms(int(os.environ.get("AZIMUTH_QA_ACTION_TIMEOUT_MS", "30000")))
...
def fixture_environment() -> dict[str, str]:
    ...
    return {
        ...
        "AZIMUTH_HOME": str(FIXTURE_HOME),
        "AZIMUTH_THUMB_CACHE_DIR": str(thumb_root),
        # Staged-import tests must never resolve the operator's real library.
        "AZIMUTH_ORIGINALS_DIR": str(FIXTURE_HOME / "import-library"),
