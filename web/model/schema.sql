...
-- The partial predicate is IN_LIBRARY spelled exactly — both halves. The
-- first spelling carried only the status half, so every count and every
-- deep walk paid a row lookup per entry to check the tail; measured at
-- 155k rows that was counts() at 1.1 s against the documented 8 ms. The
-- old names are dropped below so an existing catalog migrates on open.
