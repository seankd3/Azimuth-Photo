...
router = APIRouter()
_db_path: Callable[[], str] = lambda: db.DB_PATH
...
async def api_watched_folders():
    return {'folders': await watched_folders.list_folders(_db_path())}
...
async def api_add_watched_folder(request: Request):
    ...
    try:
        folder = await watched_folders.add_folder(
            _db_path(), body.get('path', ''), recursive=bool(body.get('recursive', True)), enabled=bool(body.get('enabled', True))
        )
...
async def api_update_watched_folder(folder_id: int, request: Request):
    ...
    folder = await watched_folders.update_folder(
        _db_path(), folder_id, enabled=_optional_bool(body, 'enabled'), recursive=_optional_bool(body, 'recursive')
    )
    if not folder:
...
async def api_remove_watched_folder(folder_id: int):
    if not await watched_folders.remove_folder(_db_path(), folder_id):
        return JSONResponse({'error': 'Watched folder not found'}, status_code=404)
...
async def api_scan_watched_folder(folder_id: int):
    try:
        result = await watched_folders.scan_folder(_db_path(), folder_id)
