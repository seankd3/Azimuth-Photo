...
def _configured_clamp_int(value, default: int, minimum: int, maximum: int) -> int:
    return request_helpers.clamp_int(value, default, minimum, maximum)
...
def _configured_normalize_search_query(query: str) -> str:
    return query_constraints.normalize_search_query(query)
...
async def map_markers_payload(
    *,
    ...
    visible_thumb_size = _visible_thumb_size_for_scope(import_batch)
    payload = await db.get_map_markers(
        orientation=orientation,
        compared=compared,
        min_stars=min_stars,
        folder=folder,
...
async def date_histogram_payload(
    *,
    ...
    search_ids, collection_id = await _resolve_collection_scope(search_ids, collection_id)
    exclude_collapsed_stack_members = _exclude_collapsed_stack_members(stacks)
    return await db.date_histogram(
        orientation=orientation,
        compared=compared,
        min_stars=min_stars,
        folder=folder,
...
async def _hidden_in_quiet_sources_count(
    *,
    ...
    if not excluded or not search_active:
        return 0
    full_total = await db.count_rankings(
        orientation=orientation,
        compared=compared,
        min_stars=min_stars,
        folder=folder,
    ...
    if quiet_total is None:
        quiet_total = await db.count_rankings(
            orientation=orientation,
            compared=compared,
            min_stars=min_stars,
            folder=folder,
...
async def api_rankings_impl(
    ...
    if sort == "taste":
        ...
        if unfiltered_taste:
            counts_task = asyncio.create_task(
                db.get_visible_pairing_pool_counts("sm", _configured_cache_root())
            )
            total_task = None
            visible_task = None
        else:
    ...
    if blend_context.get("active") and cached_blended_order is not None:
        ...
        all_rows = []
        if total_images > 0:
            all_rows = await db.get_rankings(
