...
# an attacker who invents tokens would otherwise grow this dict without limit.
# share/routes.py has bounded the same thing at 2,048 since test_hardening was
# written; this endpoint was the copy that never got it.
MAX_TRACKED_UNLOCK_TOKENS = 2048
_unlock_failures: dict[str, tuple[int, float]] = {}


def _record_unlock_failure(token: str) -> None:
    ...
    if token not in _unlock_failures and len(_unlock_failures) >= MAX_TRACKED_UNLOCK_TOKENS:
        # Evict the entry closest to expiring, so an eviction never shortens a
        # lockout that is still doing its job.
        oldest = min(_unlock_failures, key=lambda key: _unlock_failures[key][1])
