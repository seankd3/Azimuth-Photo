...
async def _ensure_model_loaded_for_config(config: dict, reason: str) -> bool:
    ...
    async with _get_model_load_lock():
        ...
        try:
            ...
            if not loaded:
                _unload_model(force=True)
                # This coroutine acquired the turn and knows its load is over.
                # The generic unload path cannot safely infer that while a load
                # is in flight, so release explicitly at this ownership boundary.
                _release_embedding_owners()
